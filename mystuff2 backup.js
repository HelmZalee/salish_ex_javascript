...
//    Incorrect values of wayPtImg are corrected below, after we have the image.      
      wayptData = {wayPoints:[], image:{height:1, width:1, halfHt:1, halfWid:1} };
      }
    ...
    if (N === 1) {position.x = 2800; position.y=1700;}//debug
    wayPoints[N] = {x:position.x, y:position.y} 
    
    top = wayPoints[N].y - wayPtImg.halfHt;
    left = wayPoints[N].x - wayPtImg.halfWid;
    wayPtId = "waypt" + N;
    ...
    if (N > 0 && btnStartBoat  && btnStartBoat.disabled) {
      ...
      $("#btnStartBoat").click(function(){
        ...
        if (typeof boatData === 'undefined') {
          boatData = {x:x, y:y, timeSec:0, speedKts:3, timeScaleSecPerHr:5, updateFreqSec:1, stopped:false};
          ...
          boatData.headingRad = Math.atan2(dy, dx);
          boatData.headingDeg = boatData.headingRad * (180/Math.PI);
          boatData.dSqToNextWpt = function(){
            var dx = boatData.el.css.
        }
        if (!boatData.el) {alert("Internal error: I lost my boat! (no id=boat)");return;}
        if (boatData.started){
          btnStartBoat.textContent = "Stop Boat";
          boatData.el.style.top = boatData.y - boatData.halfHeight + "px";
          boatData.el.style.left = boatData.x - boatData.halfWidth + "px";
        ...
        function moveBoat(boatData){
          ...
          var dx = wayPoints[N].x - (boatData.el.style.left + boatData.halfWidth);
          var dy = wayPoints[N].y - (boatData.el.style.top + boatData.halfHeight`);
          dSqToNextWpt
...
//    Connect waypoints with a line.
      ...
      for (var N = 1; N < wayPoints.length;N++){
        pos2 = {x:wayPoints[N].x, y:wayPoints[N].y};
